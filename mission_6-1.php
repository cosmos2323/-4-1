...
$result=$pdo->query('SHOW CREATE TABLE ta');
...
if(!empty($_POST['name']) && !empty($_POST['comment']) && !empty($_POST['hide'])){//名前・コメント・編集対象番号が入力されている時
   	...
   	$name2=$name;
   	$comment2=$comment;
   	$sql=$pdo->query("update ta set name='$name2',comment='$comment2' where id=$id");//編集の場所
		...
		if(!empty($_POST['comment'])){
			if(empty($_POST['hide'])){
				if($_POST['pw']==$pass){//名前・コメントが入力されていて、編集対象番号が空の時パスワード入力して送信
					...
					//投稿内容を受け取る場所
					$sql=$pdo->prepare("INSERT INTO ta (name,comment,date)VALUES(:name,:comment,:date)");//書き込み
	...
	if(!empty($_POST['edit'])){//編集対象番号が入力された時
  		 if($_POST['pw']==$pass){
			$name2=$_POST['edit'];
   			...
   			//編集の場所
			$sql='SELECT*FROM ta ORDER BY id';
...
if(!empty($_POST['delete'])){//削除対象番号が入力された時
   		if($_POST['pw']==$pass){
			$id_d=$_POST['delete'];
			$sql=$pdo->query("delete from ta where id=$id_d");
			$result=$pdo->prepare("TRUNCATE TABLE ta");
		//	$result=$pdo->query("ALTER TABLE ta AUTO_INCREMENT=1");
